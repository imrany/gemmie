...
<template>
    <div class="w-[300px] bg-white z-5 fixed top-0 left-0 bottom-0 border-r-[1px]">
        <div class="flex items-center justify-between p-3">
            <p class="font-semibold text-xl text-black">Gemini</p>
            <div class="flex gap-2 items-center">
                <button @click="props.functions.setShowInput" title="Open a new chat" class="w-[30px] h-[30px] flex items-center justify-center hover:bg-gray-100 rounded-[50px] cursor-pointer">
                    <span class="pi pi-plus-circle text-sm"></span>
                </button>
        ...
        <div v-else-if="props.data.parsedUserDetails.username===undefined||props.data.parsedUserDetails.username.length===0" class="flex flex-col items-center text-sm h-[70vh] w-full px-4 cursor-pointer justify-center">
            <form @submit="handleSubmit" class="flex gap-2 justify-center flex-col">
                ...
                <button class="rounded-md flex justify-center items-center bg-gray-800 text-white h-[37px] w-fit px-5">Submit</button>
            </form>
        </div>
        <div v-else-if="props.data.parsedUserDetails.username.length!==0" class="flex flex-col h-[70vh] px-3 text-sm justify-center">
            <div class="flex justify-center flex-col">
                ...
                <button @click="props.functions.setShowInput()" class="rounded-md flex justify-center items-center bg-gray-200 h-[40px] w-fit mt-2 px-3">Try it now</button>
            </div>
        </div>
    </div>
</template>