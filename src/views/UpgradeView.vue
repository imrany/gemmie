...
const planName = route.params.plan as
    | "student"
    | "pro"
    | "hobbyist"
    | undefined;
...
const isUsernamePrefilled = computed(() => {
    return (
        ...
        parsedUserDetails.value?.username.trim() !== ""
    );
});

const isEmailPrefilled = computed(() => {
    return (
        parsedUserDetails.value && parsedUserDetails.value?.email.trim() !== ""
    );
});

const isPhonePrefilled = computed(() => {
    return !!(
        parsedUserDetails.value?.phoneNumber &&
        parsedUserDetails.value?.phoneNumber.trim() !== ""
    );
});
...
async function handlePayment() {
    ...
    try {
        ...
        toast.dismiss(processingToast);
        toast.success(
            `M-Pesa prompt sent to ${paymentForm.phone}. Please check your phone and enter your M-Pesa PIN to complete the payment.`,
            { duration: 8000 },
        );
        ...
        let confirmationToast = toast.loading(
        ...
        paymentCheckInterval = window.setInterval(async () => {
            ...
            try {
                const ext = JSON.parse(
                    localStorage.getItem("external_reference") || '""',
                );
                if (!ext) {
                    ...
                    isProcessing.value = false;
                    toast.error("Payment reference lost. Please try again.", {
                        duration: 6000,
                    });
                    return;
                }
                ...
                if (
                    transResults?.data?.Status === "Success" ||
                    transResults?.success === true
                ) {
                    clearIntervals();
                    toast.dismiss(confirmationToast);

                    toast.success(
                        `Payment successful! Your ${selectedPlan.value?.name} plan is now active until ${expiryDate.value}.`,
                        {
                            duration: 10000,
                            important: true,
                        },
                ...
                if (
                    transResults?.data?.Status === "Failed" ||
                    transResults?.success === false
                ) {
                    clearIntervals();
                    toast.dismiss(confirmationToast);
            ...
            if (attempts >= maxAttempts) {
                ...
                toast.dismiss(confirmationToast);
                toast.error(
                    "Payment confirmation timeout. If you completed the payment, please refresh this page or contact support.",
                    {
                        duration: 10000,
                        action: {
                            label: "Refresh Page",
...
async function sendSTK(paymentData: any) {
    try {
        ...
        toast.error(
            "Failed to send M-Pesa prompt. Please check your connection and try again.",
            { duration: 5000 },
        );
        throw error;
    }
...
onMounted(() => {
    ...
    if (expiry && expiry * 1000 > Date.now()) {
        toast.info(
            `You currently have an active ${parsedUserDetails.value.planName} plan.`,
            {
                duration: Infinity,
...
<template>
    <div
        class="min-h-screen py-6 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-900 transition-colors duration-300"
    >
        <!-- Back Button -->
        <div class="flex w-full mb-6">
            <button
                @click="showCheckout ? goBackToPlans() : handleBack()"
        ...
        <!-- Active Plan Notice -->
        <div
            v-if="hasActivePlan && !showCheckout"
            class="max-w-7xl mx-auto mb-8"
        >
            <div
                class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 transition-colors duration-200"
            >
                <div class="flex items-center">
                    <Info
                        class="w-4 h-5 text-blue-600 dark:text-blue-400 mr-3"
                    />
                    <div>
                        <h3
                            class="text-blue-900 dark:text-blue-300 font-medium"
                        >
                            Current Active Plan
                        </h3>
                        <p class="text-blue-700 dark:text-blue-400 text-sm">
                            You have an active
                            {{ parsedUserDetails.planName }} plan with
                            {{ currentPlanTimeLeft }} remaining. Purchasing a
                            new plan will replace your current one.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div v-if="!showCheckout">
            <div class="max-w-7xl mx-auto text-center mb-12">
                ...
                <p class="mt-4 text-gray-600 dark:text-gray-400 text-lg">
                    Flexible pricing designed for Students, Professionals, and
                    Hobbyists.
                </p>
            </div>
                ...
                >
                    ...
                    <div class="p-6 flex-grow flex flex-col">
                        ...
                        <div class="mb-6">
                            ...
                            >
                            <span
                                class="text-gray-500 dark:text-gray-400 text-sm ml-1"
                                >{{ plan.duration }}</span
                            >
                        </div>
                        ...
                        <ul class="space-y-3 flex-grow mb-6">
                            <li
                                ...
                                :key="feature"
                                class="flex items-start text-gray-700 dark:text-gray-300 text-sm"
                            >
                                <Check
                                    class="w-4 h-4 text-green-600 dark:text-green-400 mr-3 mt-0.5"
                                />
        ...
        <!-- Checkout View -->
        <div
            v-else-if="parsedUserDetails && showCheckout"
            class="max-w-2xl mx-auto"
        >
            <div
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden transition-colors duration-300"
            >
                <!-- Header -->
                <div
                    class="bg-gradient-to-r from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-800 px-6 py-8 text-white"
                >
                    ...
                    <p class="text-blue-100 dark:text-blue-200">
                        {{
                            hasActivePlan
                                ? `Upgrading to the ${selectedPlan?.name} plan`
                ...
                <div
                    v-if="hasActivePlan"
                    class="bg-yellow-50 dark:bg-yellow-900/20 border-b border-yellow-200 dark:border-yellow-800 px-6 py-4 transition-colors duration-200"
                >
                    <div class="flex items-center">
                        <TriangleAlert
                            class="w-4 h-4 text-yellow-600 dark:text-yellow-400 mr-3"
                        />
                        <div>
                            <p
                                class="text-yellow-800 dark:text-yellow-300 text-sm"
                            >
                                <strong>Note:</strong> You currently have
                                {{ currentPlanTimeLeft }} remaining on your
                                {{ parsedUserDetails.planName }} plan. This
                                purchase will replace your current plan
                                immediately.
                            </p>
                        </div>
                    </div>
                ...
                <!-- Plan Summary -->
                <div
                    class="p-6 border-b border-gray-200 dark:border-gray-700 transition-colors duration-200"
                >
                    ...
                    <form @submit.prevent="handlePayment" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div
                                ...
                                v-if="isPhonePrefilled && !paymentForm.phone"
                            >
                                <p
                        ...
                        <div
                            class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800 transition-colors duration-200"
                        >
                            <div class="flex items-center justify-between mb-2">
                                <div>
                                    <h5
                                        class="font-medium text-blue-900 dark:text-blue-300 mb-1"
                                    >
                                        Plan Duration
                                    </h5>
                                    <p
                                    ...
                                    >
                                        Your {{ selectedPlan?.name }} plan will
                                        be active for
                                        {{ selectedPlan?.duration }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p
                                        class="text-xs text-blue-600 dark:text-blue-400 uppercase font-semibold"
                                    >
                                        Expires
                                    </p>
                                    <p
                        ...
                        <div
                            class="border border-green-200 dark:border-green-800 rounded-lg p-4 bg-green-50 dark:bg-green-900/20 transition-colors duration-200"
                        >
                            <div class="flex items-center mb-3">
                                <div
                                    class="w-8 h-8 bg-green-600 dark:bg-green-500 rounded-full flex items-center justify-center mr-3"
                                >
                                    <CardSim class="w-4 h-4 text-white" />
                                </div>
                                <div>
                                    <h5
                                        class="font-medium text-green-900 dark:text-green-300"
                                    >
                                        M-Pesa Payment
                                    </h5>
                                    <p
                                    ...
                                    >
                                        Safe and secure mobile payment
                                    </p>
                                </div>
                            </div>
                            <div
                            ...
                            >
                                <p class="mb-1">
                                    • You'll receive an M-Pesa prompt on your
                                    phone
                                </p>
                                <p class="mb-1">
                        ...
                        >
                            ...
                            <ul
                                class="text-xs text-red-700 dark:text-red-400 mt-2 ml-6"
                            >
                                <li v-if="!paymentForm.username.trim()">
                        ...
                        <div class="flex gap-4 pt-4">
                            ...
                            >
                                ...
                                {{
                                    isProcessing
                                        ? "Processing..."
                                        : `Pay ${selectedPlan?.price} Ksh via M-Pesa`
                                }}
                            </button>
                        </div>
                    </form>
