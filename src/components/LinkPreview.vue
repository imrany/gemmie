...
const isEmbeddableVideo = computed(() => {
  return props.preview.embedHtml && 
    (props.preview.videoType === 'youtube' || props.preview.videoType === 'vimeo')
})
...
const isSocialVideo = computed(() => {
  return (props.preview.videoType === 'twitter' || props.preview.videoType === 'tiktok') && 
...
const pauseVideo = () => {
  const video = document.getElementById(`${videoId.value}-video`) as HTMLVideoElement
  if (video) {
    video.pause()
...
const playSocialVideo = (url: string, videoType: string) => {
    ...
    >
        ...
        >
            ...
            <div 
                ...
                @click="playEmbeddedVideo($event.currentTarget as HTMLElement)"
            >
                <img 
                v-if="preview.videoThumbnail || preview.previewImage"
                :src="preview.videoThumbnail || preview.previewImage" 
                :alt="preview.title" 
        ...
        >
            ...
            <button 
                ...
                title="Play/Pause"
            >
                <svg v-if="!isPlaying" class="play-icon w-4 h-4 ml-0.5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M8 5v14l11-7z"/>
                </svg>
                <svg v-else class="pause-icon w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
                </svg>
            </button>
            <button 
        ...
        <div 
            ...
            @click="playSocialVideo(preview.url, preview.videoType!)"
        >
            <img 
