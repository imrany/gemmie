...
const { currentChatId, screenWidth, isOnline, loadChat } = inject(
    "globalState",
) as {
    currentChatId: Ref<string>;
    loadChat: () => Promise<[boolean, string]>;
    ...
    >
        ...
        >
            ...
            <p
                class="max-sm:max-w-xs sm:text-sm text-xs text-gray-600 dark:text-gray-400"
            >
                It is neither stored locally or fetched, get connected and try
                again.
            </p>
            <div class="flex gap-3 mt-4">
                <Button
                    @click="
                        async () => {
                            const [isLoaded, message] = await loadChat();
                            if (isLoaded) {
                                toast.info(message);
                            } else {
                                toast.error(message);
