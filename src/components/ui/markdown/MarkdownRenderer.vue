// ==========================================
// MarkdownRenderer.vue - Main Component
// ==========================================
...
const processImages = (text: string): string => {
  ...
  // Process all image variations and store them
  let result = text
...
const processCallouts = (text: string): string => {
  ...
  return text.replace(
    ...
    (match, type, content) => {
      ...
      callouts.value.push({
        id,
        type: type as 'NOTE' | 'TIP' | 'IMPORTANT' | 'WARNING' | 'CAUTION',
...
const processBasicMarkdown = (text: string): string => {
  ...
  // Horizontal rules
  html = html.replace(/^(---|\*\*\*|___)$/gm, '<hr class="border-t border-gray-300 dark:border-gray-600 my-3">')
  ...
  // Lists
  html = html.replace(/^[\*\-\+] (.+$)/gm, '<li class="ml-3 leading-relaxed">• $1</li>')
  html = html.replace(/^\d+\. (.+$)/gm, '<li class="ml-3 list-decimal leading-relaxed">$1</li>')
...
const renderPart = (part: string) => {
  if (part.includes('___CODE_BLOCK_')) {
    return 'code-block'
  } else if (part.includes('___IMAGE_')) {
    return 'image'
  } else if (part.includes('___TABLE_')) {
    return 'table'
  } else if (part.includes('___CALLOUT_')) {
    return 'callout'
  } else if (part.includes('___LINK_')) {
    return 'link'
  }
  return 'html'
}
